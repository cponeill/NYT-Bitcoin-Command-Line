...
from nyt_most_popular21 import reviews21
...
if __name__ == '__main__':
    if 'daemon' not in sys.argv:
        pid_file = './nyt-most-popular21.pid'
        if os.path.isfile(pid_file):
            ...
            try:
                ...
                pass
        try:
            p = subprocess.Popen(['python3', 'nyt-most-popular21-server.py', 'daemon'])
            open(pid_file, 'w').write(str(p.pid))
        except subprocess.CalledProcessError:
            raise ValueError("error starting nyt-most-popular21-server.py daemon")
    else:
